...
| **Velocity hard-clamp** | 0.25 m/s global, 0.20 m/s during centering | Derived from the 270 ms latency budget: 0.25 m/s × 0.27 s ≈ 6.75 cm worst-case error before the next correction — 2.4× safety factor against the ±30 px centering tolerance |
| **Transit by dead reckoning** | time × commanded velocity, no GPS | Known failure modes documented below |
...
**Results: mAP@0.5 = 0.969 · F1 = 0.94 @ 0.745 confidence · recall 0.98.** Training curves, PR curves, and confusion matrices are in [`docs/training_results/`](docs/training_results/).

**2. Proxy-target detector (deployed in flight).** The field demo could not be flown at campus scale, so colored boxes served as proxy landmarks and a second model was trained for them: **350 images, 80 epochs, mAP 0.83**, deployed via the Ultralytics Docker container on the Jetson.
...
| Metric | Result |
|---|---|
| Yaw drift | < 2° over a 24 s rotation |
...
- Started on a SpeedyBee F405 V4; its faulty barometer backfed 5 V onto the I²C bus, destroying the board → diagnosed and migrated to the H743.
...
├── model/
│   └── best.pt               # deployed proxy-target YOLOv8-Nano weights
├── docs/
│   ├── Progress_Report_1.pdf
│   ├── Progress_Report_2.pdf
│   ├── brochure.png          # project brochure (presented under working title "Intellifly")
│   ├── training_results/     # Ultralytics curves, confusion matrices, batch previews
│   └── photos/               # drone, field demo, FYP award
└── README.md
```
...
GIK Institute of Engineering Sciences and Technology · Final Year Project · 2026
